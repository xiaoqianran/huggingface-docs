/**
 * huggingface Docs chrome — paradigm v3 (aligned to modal-docs)
...
(function () {
  ...
  const STORE_KEY = "hf-docs-nav-v3";
  const LANG_KEY = "hf-docs-lang";
  ...
  if (active) {
    ...
    if (!anyRestored) {
      const preferred =
        nav?.querySelector('.track[data-track="transformers"]') ||
        nav?.querySelector('.track[data-track="hub"]') ||
