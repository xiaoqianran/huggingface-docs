...
Pass the text query to the processor and return the indexed text embeddings from the model. For image-to-text search, replace the `text` parameter in [ColPaliProcessor](/docs/transformers/v5.17.0/en/model_doc/colpali#transformers.ColPaliProcessor) with the `images` parameter to pass images.
...
https://huggingface.co/docs/transformers/v5.17.0/tasks/zero_shot_image_classification.md
