...
- run inference with [Pipeline](/docs/transformers/v5.17.0/en/main_classes/pipelines#transformers.Pipeline)
- fine-tune a model with [Trainer](/docs/transformers/v5.17.0/en/main_classes/trainer#transformers.Trainer)
...
Paste your User Access Token into [notebook_login](https://huggingface.co/docs/huggingface_hub/v1.30.0/en/package_reference/authentication#huggingface_hub.notebook_login) when prompted to log in.
...
| **Class** | **Description** |
|---|---|
| [PreTrainedConfig](/docs/transformers/v5.17.0/en/main_classes/configuration#transformers.PreTrainedConfig) | A file that specifies a model's attributes such as the number of attention heads or vocabulary size. |
| [PreTrainedModel](/docs/transformers/v5.17.0/en/main_classes/model#transformers.PreTrainedModel) | A model (or architecture) defined by the model attributes from the configuration file. A pretrained model only returns the raw hidden states. For a specific task, use the appropriate model head to convert the raw hidden states into a meaningful result (for example, [LlamaModel](/docs/transformers/v5.17.0/en/model_doc/llama2#transformers.LlamaModel) versus [LlamaForCausalLM](/docs/transformers/v5.17.0/en/model_doc/llama2#transformers.LlamaForCausalLM)). |
| Preprocessor | A class for converting raw inputs (text, images, audio, multimodal) into numerical inputs to the model. For example, [PreTrainedTokenizer](/docs/transformers/v5.17.0/en/main_classes/tokenizer#transformers.PythonBackend) converts text into tensors and [ImageProcessingMixin](/docs/transformers/v5.17.0/en/main_classes/image_processor#transformers.ImageProcessingMixin) converts pixels into tensors. |
...
Use [from_pretrained()](/docs/transformers/v5.17.0/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) to load the weights and configuration file from the Hub into the model and preprocessor class.
...
For inference, pass the tokenized inputs to [generate()](/docs/transformers/v5.17.0/en/main_classes/text_generation#transformers.GenerationMixin.generate) to generate text. Decode the token ids back into text with [batch_decode()](/docs/transformers/v5.17.0/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.batch_decode).
...
The [Pipeline](/docs/transformers/v5.17.0/en/main_classes/pipelines#transformers.Pipeline) class is the most convenient way to run inference with a pretrained model. It supports many tasks such as text generation, image segmentation, automatic speech recognition, document question answering, and more.
...
Prompt [Pipeline](/docs/transformers/v5.17.0/en/main_classes/pipelines#transformers.Pipeline) with some initial text to generate more text.
...
Pass an image - a URL or local path to the image - to [Pipeline](/docs/transformers/v5.17.0/en/main_classes/pipelines#transformers.Pipeline).
...
Pass an audio file to [Pipeline](/docs/transformers/v5.17.0/en/main_classes/pipelines#transformers.Pipeline).
...
Next, set up [TrainingArguments](/docs/transformers/v5.17.0/en/main_classes/trainer#transformers.TrainingArguments) with the training features and hyperparameters.
...
Finally, pass all these separate components to [Trainer](/docs/transformers/v5.17.0/en/main_classes/trainer#transformers.Trainer) and call [train()](/docs/transformers/v5.17.0/en/main_classes/trainer#transformers.Trainer.train) to start.
...
Share your model and tokenizer to the Hub with [push_to_hub()](/docs/transformers/v5.17.0/en/main_classes/trainer#transformers.Trainer.push_to_hub).
...
- **Inference**: Explore the [Pipeline](/docs/transformers/v5.17.0/en/main_classes/pipelines#transformers.Pipeline) further, inference and chatting with LLMs, agents, and how to optimize inference with your machine learning framework and hardware.
- **Training**: Study the [Trainer](/docs/transformers/v5.17.0/en/main_classes/trainer#transformers.Trainer) in more detail, as well as distributed training and optimizing training on specific hardware.
...
### How caching works
https://huggingface.co/docs/transformers/v5.17.0/cache_explanation.md
