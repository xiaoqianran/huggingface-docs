...
This is the configuration class to store the configuration of a Qwen4ExpModel. It is used to instantiate a Qwen4 Exp
...
## Qwen4ExpTextConfig[[transformers.Qwen4ExpTextConfig]]
...
hc_count (`int`, *optional*, defaults to 4) : Number of residual streams used by the hyper-connections.

hc_lowrank (`int`, *optional*, defaults to 320) : Rank of the learned hyper-connection input mixer.

ple_layer_ids (`list[int]`, *optional*) : One-indexed decoder layer ids that use Per-Layer Embedding (PLE).

ple_embed_dim (`int`, *optional*) : Total dimension of the embeddings concatenated from all n-gram heads in each PLE module. Defaults to `hidden_size`.

ple_conv_kernel_size (`int`, *optional*, defaults to 4) : Kernel size of the dilated depthwise convolution in each PLE module.

ngram_size (`int`, *optional*, defaults to 3) : Largest token n-gram represented by PLE.

heads_per_ngram (`int`, *optional*, defaults to 8) : Number of independently hashed embedding heads for every n-gram order.

ngram_vocab_size_base (`int`, *optional*, defaults to 20000000) : Lower bound used to derive a distinct prime vocabulary size for each hashed n-gram head.

make_ngram_vocab_size_divisible_by (`int`, *optional*, defaults to 128) : Divisor used to pad the combined n-gram embedding vocabulary.

seed (`int`, *optional*, defaults to 1234) : Seed used to deterministically derive the per-layer n-gram hash multipliers.

split_ngram_parts (`int`, *optional*, defaults to 512) : Number of checkpoint shards used for each PLE n-gram embedding table. Loading concatenates the shards into a single runtime embedding, while `save_pretrained` restores the configured sharded layout.

indexer_n_heads (`int`, *optional*) : Number of query heads used by the QSA token indexer. Setting this enables QSA on full-attention layers.

indexer_kv_heads (`int`, *optional*) : Number of indexer key heads. Qwen4-Exp QSA requires one key head.

indexer_head_dim (`int`, *optional*) : Dimension of every QSA indexer query and key head.

indexer_budget (`int`, *optional*) : Maximum number of tokens selected from complete compressed blocks for each query.

indexer_compress_ratio (`int`, *optional*) : Number of consecutive token keys averaged into one QSA index block.

norm_topk_prob (`bool`, *optional*, defaults to `True`) : Whether to normalize the weights of the routed experts. 

output_gate_type (`str`, *optional*) : Activation used by the output gate of linear attention. If unset, `hidden_act` is used.
...
## Qwen4ExpVisionConfig[[transformers.Qwen4ExpVisionConfig]]
...
#### transformers.Qwen4ExpVisionModel[[transformers.Qwen4ExpVisionModel]]
...
#### forward[[transformers.Qwen4ExpVisionModel.forward]]
...
## Qwen4ExpTextModel[[transformers.Qwen4ExpTextModel]]

#### transformers.Qwen4ExpTextModel[[transformers.Qwen4ExpTextModel]]
...
#### forward[[transformers.Qwen4ExpTextModel.forward]]
...
## Qwen4ExpModel[[transformers.Qwen4ExpModel]]

#### transformers.Qwen4ExpModel[[transformers.Qwen4ExpModel]]
...
## Qwen4ExpForCausalLM[[transformers.Qwen4ExpForCausalLM]]

#### transformers.Qwen4ExpForCausalLM[[transformers.Qwen4ExpForCausalLM]]
...
#### forward[[transformers.Qwen4ExpForCausalLM.forward]]
...
## Qwen4ExpForConditionalGeneration[[transformers.Qwen4ExpForConditionalGeneration]]

#### transformers.Qwen4ExpForConditionalGeneration[[transformers.Qwen4ExpForConditionalGeneration]]
