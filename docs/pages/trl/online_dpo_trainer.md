...
To better understand your model's behavior during training, you can log sample completions periodically using the [LogCompletionsCallback](/docs/trl/v1.13.0/en/callbacks#trl.LogCompletionsCallback).
...
* `beta`: The parameter that controls the weight of the loss term representing the deviation from the reference model. Typically fixed, but can be made dynamic by passing a list to [experimental.online_dpo.OnlineDPOConfig](/docs/trl/v1.13.0/en/online_dpo_trainer#trl.experimental.online_dpo.OnlineDPOConfig).
...
[Source](https://github.com/huggingface/trl/blob/v1.13.0/trl/experimental/online_dpo/online_dpo_config.py#L23)
...
model_init_kwargs (`dict[str, Any]`, *optional*) : Keyword arguments to pass to `AutoModelForCausalLM.from_pretrained` when instantiating the model from a string. The `revision` value is also used when loading reward processing classes.
...
Configuration class for the [experimental.online_dpo.OnlineDPOTrainer](/docs/trl/v1.13.0/en/online_dpo_trainer#trl.experimental.online_dpo.OnlineDPOTrainer).
...
https://huggingface.co/docs/trl/v1.13.0/liger_kernel_integration.md
