...
[experimental.async_distillation.AsyncDistillationTrainer](/docs/trl/v1.14.0/en/async_distillation_trainer#trl.experimental.async_distillation.AsyncDistillationTrainer) is the async counterpart to [DistillationTrainer](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationTrainer),
architected like [experimental.async_grpo.AsyncGRPOTrainer](/docs/trl/v1.14.0/en/async_grpo_trainer#trl.experimental.async_grpo.AsyncGRPOTrainer): a background rollout worker generates the student's own on-policy
...
objective [DistillationTrainer](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationTrainer) and
...
already exist (e.g. trained separately with [GRPOTrainer](/docs/trl/v1.14.0/en/grpo_trainer#trl.GRPOTrainer)/[RLOOTrainer](/docs/trl/v1.14.0/en/rloo_trainer#trl.RLOOTrainer)) and be served over HTTP before you
...
## How it differs from [DistillationTrainer](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationTrainer)

In [DistillationTrainer](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationTrainer), the teacher is a locally loaded model: generation,
teacher forward pass, and the gradient update all happen sequentially in the same process.
[experimental.async_distillation.AsyncDistillationTrainer](/docs/trl/v1.14.0/en/async_distillation_trainer#trl.experimental.async_distillation.AsyncDistillationTrainer) separates these concerns the same way [experimental.async_grpo.AsyncGRPOTrainer](/docs/trl/v1.14.0/en/async_grpo_trainer#trl.experimental.async_grpo.AsyncGRPOTrainer) separates GRPO's rollout
from its update:
...
distribution is ever transmitted (`teacher_top_k`), not the full vocabulary — see [experimental.async_distillation.AsyncDistillationConfig](/docs/trl/v1.14.0/en/async_distillation_trainer#trl.experimental.async_distillation.AsyncDistillationConfig)'s
...
NCCL. As with [experimental.async_grpo.AsyncGRPOTrainer](/docs/trl/v1.14.0/en/async_grpo_trainer#trl.experimental.async_grpo.AsyncGRPOTrainer), generation runs ahead of training, so samples may reflect a slightly stale
...
[Source](https://github.com/huggingface/trl/blob/v1.14.0/trl/experimental/async_distillation/async_distillation_config.py#L22)
...
beta (`float`, *optional*, defaults to `0.0`) : Interpolation coefficient for the generalized Jensen-Shannon Divergence. `0.0` is forward KL (mean-seeking), `1.0` is reverse KL (mode-seeking), and values in between interpolate, following the same generalized-JSD formulation [DistillationTrainer](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationTrainer) computes internally. The support the divergence is computed over differs by regime, mirroring `ServerDistillationTrainer`: at `beta=0.0`, the full `teacher_top_k`-wide teacher-reported support (plus tail bucket) is used, since forward KL's weighting is exactly what that support provides. At `beta != 0.0`, the support is narrowed to just two candidates — the teacher's own top-1 token and the completion's actual/realized token — since those are the only two token identities the wire protocol guarantees a teacher logprob for without transmitting a wider (or full) vocabulary; anything wider would only be a probabilistic approximation of covering the student's own likely tokens, not a guarantee.
...
Configuration class for the [experimental.async_distillation.AsyncDistillationTrainer](/docs/trl/v1.14.0/en/async_distillation_trainer#trl.experimental.async_distillation.AsyncDistillationTrainer).
...
[AsyncGRPOConfig](/docs/trl/v1.14.0/en/async_grpo_trainer#trl.experimental.async_grpo.AsyncGRPOConfig) (async pipeline, vLLM server, logging fields are the same), with
...
[Source](https://github.com/huggingface/trl/blob/v1.14.0/trl/experimental/async_distillation/async_distillation_trainer.py#L872)
...
args ([experimental.async_distillation.AsyncDistillationConfig](/docs/trl/v1.14.0/en/async_distillation_trainer#trl.experimental.async_distillation.AsyncDistillationConfig), *optional*) : Configuration for this trainer. If `None`, a default configuration is used.
...
Async counterpart to [DistillationTrainer](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationTrainer), architected exactly like
[AsyncGRPOTrainer](/docs/trl/v1.14.0/en/async_grpo_trainer#trl.experimental.async_grpo.AsyncGRPOTrainer): a background rollout worker generates the student's on-policy
...
### Reward Modeling
https://huggingface.co/docs/trl/v1.14.0/reward_trainer.md
