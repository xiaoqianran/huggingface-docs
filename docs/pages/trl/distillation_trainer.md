...
> [!TIP]
> Depending on the model size and the overall GPU memory requirements for training, you may need to adjust the `vllm_gpu_memory_utilization` parameter in [DistillationConfig](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationConfig) to avoid underutilization or out-of-memory errors.
...
Use `max_tool_calling_iterations` in the [DistillationConfig](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationConfig) to cap the number of tool-calling turns. By default there is no limit, and generation stops when the student produces a response turn with no tool calls.
...
[Source](https://github.com/huggingface/trl/blob/v1.14.0/trl/trainer/distillation_trainer.py#L294)
...
args ([DistillationConfig](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationConfig), *optional*) : Configuration for this trainer. If `None`, a default configuration is used.
...
[Source](https://github.com/huggingface/trl/blob/v1.14.0/trl/trainer/distillation_config.py#L22)
...
Configuration class for the [DistillationTrainer](/docs/trl/v1.14.0/en/distillation_trainer#trl.DistillationTrainer).
...
### Reward Functions
https://huggingface.co/docs/trl/v1.14.0/rewards.md
