...
3. **Seamless integration with GKD** – GOLD inherits the on-policy vs. off-policy scheduling from the [experimental.gkd.GKDTrainer](/docs/trl/v1.14.0/en/gkd_trainer#trl.experimental.gkd.GKDTrainer), so you can combine sequence-level KD, generalized JSD, and cross-tokenizer distillation in a single training run.
...
The [experimental.gold.GOLDTrainer](/docs/trl/v1.14.0/en/gold_trainer#trl.experimental.gold.GOLDTrainer) subclasses [SFTTrainer](/docs/trl/v1.14.0/en/sft_trainer#trl.SFTTrainer) and accepts the same datasets as other TRL trainers (lists of ChatML style
messages). Important configuration flags on [experimental.gold.GOLDConfig](/docs/trl/v1.14.0/en/gold_trainer#trl.experimental.gold.GOLDConfig) include:
...
* `beta`, `lmbda`, `seq_kd` – inherited from [experimental.gkd.GKDConfig](/docs/trl/v1.14.0/en/gkd_trainer#trl.experimental.gkd.GKDConfig), controlling the generalized JSD interpolation and on-policy
  sampling ratio.
...
> [!TIP]
> For VLMs, `truncation_mode='keep_end'` is not supported because image tokens reside in the prompt portion of the sequence and may be silently dropped. Use `truncation_mode='keep_start'` (the default) or set `max_length=None` in the [experimental.gold.GOLDConfig](/docs/trl/v1.14.0/en/gold_trainer#trl.experimental.gold.GOLDConfig). This allows the model to process the full sequence length without truncating image tokens.
>
> ```python
...
[Source](https://github.com/huggingface/trl/blob/v1.14.0/trl/experimental/gold/gold_trainer.py#L770)
...
[Source](https://github.com/huggingface/trl/blob/v1.14.0/trl/experimental/gold/gold_trainer.py#L2578)
...
teacher_model_init_kwargs (`dict[str, Any]`, *optional*) : Keyword arguments to pass to `AutoModelForCausalLM.from_pretrained` when instantiating the teacher model from a string. The `revision` value is also used when loading the teacher processing class.
...
Configuration class for [experimental.gold.GOLDTrainer](/docs/trl/v1.14.0/en/gold_trainer#trl.experimental.gold.GOLDTrainer).

This class includes only the parameters that are specific to GOLD training. For a full list of training arguments,
please refer to the [TrainingArguments](https://huggingface.co/docs/transformers/v5.17.0/en/main_classes/trainer#transformers.TrainingArguments) and [SFTConfig](/docs/trl/v1.14.0/en/sft_trainer#trl.SFTConfig) documentation.
...
### DPO Trainer
https://huggingface.co/docs/trl/v1.14.0/dpo_trainer.md
