...
1. 创建一个 Hugging Face [access token](https://huggingface.co/settings/tokens)，并对目标存储库具有 **写入** 权限。为了获得更好的安全性，请使用范围仅限于您要同步到的存储库的[fine-grained token](https://huggingface.co/settings/tokens)。
