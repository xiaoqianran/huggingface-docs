...
您可以使用信用卡或 AWS 或 Google Cloud 帐户支付团队订阅费用，也可以通过年度合同升级到企业版。
...
Hugging Face 使用 [Stripe](https://stripe.com) 安全地处理您的付款信息。Hugging Face 计算服务通过信用卡支付。您可以从账单设置中将信用卡添加到您的帐户。

组织也可以通过 [cloud marketplace](#cloud-providers-partnerships) 订阅进行计费，这样 Hugging Face 的使用情况就会显示在其 AWS 或 Google Cloud 账单上。
...
您还可以选择通过云市场将您的 Hugging Face 组织链接到您的云提供商帐户。 Hugging Face 计算服务的使用量将包含在您的云提供商账单中，而不是从信用卡中扣除。

支持两个市场：

* **AWS Marketplace** — 从 [Hugging Face Platform listing](https://aws.amazon.com/marketplace/pp/prodview-n6vsyhdjkfng2) 订阅以链接您的 AWS 账户。请阅读我们的[blog post](https://huggingface.co/blog/aws-marketplace)了解更多信息。
* **Google Cloud Marketplace** — 从 [Hugging Face Hub listing](https://console.cloud.google.com/marketplace/product/huggingface-public/hugging-face-hub) 订阅以链接您的 Google Cloud 帐户。在这两种情况下，您都从市场列表开始，然后选择要链接的 Hugging Face 组织。链接后，组织的付款方式将显示在其计费设置中，显示链接的云帐户和当前的订阅状态。

云市场订阅必须先获得 Hugging Face 团队的批准，然后才能开始通过云市场订阅付费。在订阅生效并获得批准之前，组织的计费设置会将其显示为待处理，并且使用量将继续按组织的现有付款方式计费。订阅验证后，您将收到一封电子邮件通知。

云市场计费仅适用于组织，并取代信用卡作为组织的付款方式。当组织通过云市场计费时，无法添加积分：而是对使用情况进行计量并向您的云提供商报告。云市场帐户的链接和取消链接记录在您组织的 [audit logs](./audit-logs) 中。

## 支持常见问题解答 

**问。为什么我需要添加学分？我可以用它们做什么？**

A. 积分可让您使用 HF 按量付费服务：- 作业：在 GPU 上运行任何工作负载
...
如果您有 PRO、Team 或 Enterprise 订阅，则订阅中包含的积分每月都会记入，并计入相同的服务。在对任何即用即付使用量进行计费之前，它们会自动应用。

**问。如果我用完积分会怎样？**

答：我们建议启用自动充值，以避免积分用完后服务中断。

**问。我在添加卡时遇到问题。这是怎么回事？**答：请确保该卡支持 3D 安全身份验证，并针对定期在线付款进行了正确配置。我们尚不支持在印度发行的信用卡，因为我们正在努力增加系统对最新 RBI 指令的合规性。在我们添加对印度信用卡的支持之前，您可以：
* 通过 [cloud marketplace](#cloud-providers-partnerships) 将组织帐户链接到 AWS 或 Google Cloud 帐户，以便访问即用即付功能（端点、空间、AutoTrain）。对于 AWS，请参阅 [Hugging Face Platform on the AWS Marketplace: Pay with your AWS Account](https://huggingface.co/blog/aws-marketplace)
* 使用其他国家发行的信用卡
...
A. 在此处查看和下载所有发票：https://huggingface.co/settings/billing/invoices。发票也通过电子邮件发送。
