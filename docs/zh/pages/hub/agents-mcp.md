...
|------|-------------|
| **Contribute Repos** |创建文件并将其写入这些已创建的存储库 |
| **Sandboxes** |创建和使用沙箱：包括沙箱文件管理 |
| **Run and Manage Jobs** |在 Hugging Face 基础设施上运行、监控和安排作业。 |

> [!提示]
> 不时添加实验工具。 Try them and feed back
