...
[JupyterLab](https://jupyter.org/) 是一个基于 Web 的 Jupyter 笔记本、代码和数据的交互式开发环境。 It is a great tool for data science and machine learning, and it is widely used by the community.通过 Hugging Face Spaces，您可以部署自己的 JupyterLab 实例，并直接从 Hugging Face 网站使用它进行开发。
...
* **硬件**：您想要用于 JupyterLab 实例的硬件。 This goes from CPUs to H100s.* 您可以选择配置 `JUPYTER_TOKEN` 密码来保护您的 JupyterLab 工作区。未指定时，默认为`huggingface`。如果您的空间是公共的或者空间位于组织中，我们强烈建议您进行此设置。
