...
在论文 [Geometric-Mean Policy Optimization](https://huggingface.co/papers/2507.20673) 中，作者提出了一种 GRPO 变体，可以最大化标记级重要性比率的“几何”平均值，而不是算术平均值。由于几何平均值对异常值比率的敏感度要低得多，因此策略更新更加稳定并且可以容忍更广泛的裁剪范围。在对数空间中对每个令牌应用剪裁，并根据优势符号（标准 PPO 信任区域）进行单边剪裁 - 至关重要的是，*在*取几何平均值之前。

要使用 GMPO，可以使用 `trl.experimental.gmpo` 中的 `GMPOTrainer` 类。
...
在 GMPO 中，在取几何平均值之前，对每个 token *log* 重要性比率（即在对数空间中）进行裁剪，因此 `epsilon` 和 `epsilon_high` 在对数空间中表示：有效比率裁剪范围是 `(exp(-epsilon), exp(epsilon_high))`。该论文建议使用比 GRPO/DAPO 明显更宽的范围，`(exp(-0.4), exp(0.4))`，以鼓励探索。

## GMPOTrainer[[trl.experimental.gmpo.GMPOTrainer]]

几何平均策略优化 (GMPO) 培训师。GMPO (https://huggingface.co/papers/2507.20673) 是一种 GRPO 变体，它最大化了
令牌级别的重要性比率而不是算术平均值。因为几何平均数对
异常值比率，策略更新更加稳定，并且可以使用更广泛的裁剪范围。

唯一的变化是[GRPOTrainer](/docs/trl/v1.9.2/en/gspo_token#trl.GRPOTrainer) 是`_compute_loss`。其他一切（生成、奖励计算、权重
同步、指标记录）继承不变
...
- **ignore_keys_for_eval** (`list[str]`，*可选*) --
  模型输出中的键列表（如果它是字典），在以下情况下应忽略这些键：收集训练期间评估的预测。`~trainer_utils.TrainOutput`包含全局步数、训练损失和指标的对象。
