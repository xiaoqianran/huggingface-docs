...
PyTorch 的分布式模块通过系统中所有 GPU 之间的来回通信来运行。
...
```
在 Accelerate 中，当调用 [prepare()](/docs/accelerate/v1.15.0/en/package_reference/accelerator#accelerate.Accelerator.prepare) 并传入模型时，此转换会自动发生。
...
在特定的点上，并且这些也必须大致在同一时间发生，然后才能继续。最直接的例子是当你通过更新模型参数时
`optimizer.step()`。
...
由于在对这些批次进行训练时跳过模型参数更新，因此在实际调用 `optimizer.step()` 之前不需要同步它们的梯度。 
...
`ddp_model.no_sync` 被 [no_sync()](/docs/accelerate/v1.15.0/en/package_reference/accelerator#accelerate.Accelerator.no_sync) 取代，操作方式相同：
...
正如您所期望的，[accumulate()](/docs/accelerate/v1.15.0/en/package_reference/accelerator#accelerate.Accelerator.accumulate) 函数通过跟踪当前批次号来围绕此条件检查，为您留下最终的结果
梯度累积API：
...
- `accumulate`：正确使用[accumulate()](/docs/accelerate/v1.15.0/en/package_reference/accelerator#accelerate.Accelerator.accumulate)
...
正如您所看到的，如果您不小心设置梯度同步，您在训练期间可能会出现超过 2 倍的减速！如果您担心确保一切都正确完成，我们强烈建议使用 [accumulate()](/docs/accelerate/v1.15.0/en/package_reference/accelerator#accelerate.Accelerator.accumulate) 函数并传入
`gradient_accumulation_steps` 或 `gradient_accumulation_plugin` 到 [Accelerator](/docs/accelerate/v1.15.0/en/package_reference/accelerator#accelerate.Accelerator) 对象，以便 Accelerate 可以为您处理此问题。

### `no_sync` 使用 FSDP 时需要额外的 GPU 内存
...
因此，在使用 FSDP 时，在内存密集型情况下，我们建议将 [GradientAccumulationPlugin](/docs/accelerate/v1.15.0/en/package_reference/utilities#accelerate.utils.GradientAccumulationPlugin) 中的 `sync_each_batch` 设置为 `True` 以禁用 `no_sync`。
