...
## 重量绑定常见的语言模型设计是将嵌入层和 softmax 层的权重联系起来。但是，将模型移动到 TPU（您自己或将其传递给 [prepare()](/docs/accelerate/v1.15.0/en/package_reference/accelerator#accelerate.Accelerator.prepare) 方法）会破坏权重绑定，您需要重新绑定权重。

要在 TPU 脚本中添加特殊行为（如权重绑定），请首先将 `distributed_type` 设置为 `DistributedType.TPU`。然后你可以使用[tie_weights](https://huggingface.co/docs/transformers/v5.16.1/en/main_classes/model#transformers.PreTrainedModel.tie_weights)方法来绑定权重。
