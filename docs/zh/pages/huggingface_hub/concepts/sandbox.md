...
- **下载，带有挂载回退。** 快速路径使用 `wget` 或 `curl`（每个常见基础映像都附带一个）从 HF CDN 下载二进制文件，速度快且免费。作为安全网，服务器的 Hub 存储库也作为卷安装在每个作业上：如果映像既没有 `wget` 也没有 `curl`，则脚本会从该安装中复制二进制文件。安装是透明的——除非实际读取，否则不需要任何成本——但读取它会增加大约 2-3 秒的冷启动时间，因此它不是默认设置。对图像唯一的硬性要求是`/bin/sh`。
...
- **HF 令牌永远不会进入沙箱**（除非您选择使用`forward_hf_token=True`），因此内部运行的代码无法泄露您的凭据。
...
## 池：一项作业中有多个沙箱 (`SandboxPool`)典型的 RL 部署或工具执行沙箱需要几 MB 的 RAM 和一个核心几秒钟。为每个 2 个 vCPU 的虚拟机和 6 秒的冷启动付费——并触发 1000 个虚拟机的调度突发——是错误的交易。因此，[SandboxPool](/docs/huggingface_hub/v1.27.0/en/package_reference/sandbox#huggingface_hub.SandboxPool)作为主机运行一个作业，并在其中复用许多沙箱。

池化沙箱不是嵌套的虚拟机或容器。这是经典的 Unix 多用户原语：

- **专用 uid** (≥ 20000),
- 该 uid 拥有的 **私人 `0700` 住宅**，
- 命令`exec`'d作为具有**清理环境**的uid（`env_clear`，因此主机的秘密永远不会泄漏），`NO_NEW_PRIVS`，每个进程**rlimits**和每个沙箱**内锁规则集**。
...
>
> - **资源 DoS。** 如果没有 cgroup 委派，CPU/总 RAM/磁盘不会分区。 `RLIMIT_NPROC` 和 `RLIMIT_AS` 限制了每个进程的使用，但激进的沙箱仍然可能使其邻居挨饿或引发全局 OOM 杀手。
> - **进程列表元数据。** 沙箱可以通过 `/proc`（名称、命令行）查看其他进程 - 它只是无法读取或向它们发出信号。隐藏它们需要一个 PID 命名空间，`unshare` 无法在此处创建。
>
> 简而言之：强制执行池化沙箱之间的机密性和完整性；仅共享可用性 (DoS) 和进程列表元数据。这是一个用户自己的并行工作负载的正确边界。对于相互敌对的不可信代码（或 GPU），请使用[Sandbox.create()](/docs/huggingface_hub/v1.27.0/en/package_reference/sandbox#huggingface_hub.Sandbox.create)，它为每个沙箱提供了自己的虚拟机。
...
缓存之所以安全，正是因为它永远不会被信任为事实：- **永远不是事实来源。** 工作中的服务器在容量方面保持权威，因此陈旧的 `live` 计数只会导致浪费的请求，而不会导致正确性。
...
| 1000 | 1000 20 | 7.4秒| 4.2秒| 4.2秒| **15.8 秒** |在大约 16 秒内创建、执行和终止 1000 个沙箱，大约需要一台主机冷启动（大约 6 秒），所有沙箱均摊 — 总计约为 **$0.0009**（20 × `cpu-basic`），而每个沙箱一个作业的 1000 个虚拟机调度突发约为 0.06 美元。服务器端创建/执行/删除每个〜1ms；预算完全是网络往返。
...
|手卷HTTP/1.1 |最小框架缓冲分块响应并中断实时流传输（已验证）||无状态 HMAC 身份验证 |从任何地方重新连接； HF 代币永远不会进入沙箱 |
