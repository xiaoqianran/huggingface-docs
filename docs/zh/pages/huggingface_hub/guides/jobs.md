...
[run_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.run_job) 允许您在 Hugging Face 的基础设施上运行任何命令：
...
> [!警告]
> **重要**：作业有默认超时（30 分钟），之后它们将自动停止。对于模型训练等长时间运行的任务，请确保使用 `timeout` 参数设置自定义超时。详情请参阅[Configure Job Timeout](#configure-job-timeout)。[run_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.run_job) 返回[JobInfo](/docs/huggingface_hub/v1.27.0/en/package_reference/jobs#huggingface_hub.JobInfo)，其中包含 Hugging Face 上作业的 URL，您可以在其中查看作业状态和日志。
保存[JobInfo](/docs/huggingface_hub/v1.27.0/en/package_reference/jobs#huggingface_hub.JobInfo)的作业ID以管理作业：
...
作业在后台运行。下一节将引导您通过 [inspect_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.inspect_job) 了解作业状态、[fetch_job_logs()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.fetch_job_logs) 查看日志以及 [fetch_job_metrics()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.fetch_job_metrics) 监控资源使用情况。
...
使用 [wait_for_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.wait_for_job) 进行阻塞，直到作业到达终止阶段（`COMPLETED`、`CANCELED`、`ERROR` 或 `DELETED`）。最终的[JobInfo](/docs/huggingface_hub/v1.27.0/en/package_reference/jobs#huggingface_hub.JobInfo)总是返回——失败的作业不会引发异常——所以检查`job.status.stage`以对结果采取行动。传递作业 ID 列表以立即等待整批作业。
...
使用 [Volume](/docs/huggingface_hub/v1.27.0/en/package_reference/jobs#huggingface_hub.Volume) 列表在作业磁盘上安装卷。
...
### 挂载本地数据要针对计算机上的数据运行作业，请使用[sync_job_volume()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.sync_job_volume)：它将本地目录同步到您的`jobs-artifacts`[Storage Bucket](https://huggingface.co/docs/hub/storage-buckets)（如果需要，会自动创建）并返回准备安装的[Volume](/docs/huggingface_hub/v1.27.0/en/package_reference/jobs#huggingface_hub.Volume)：
...
每个目录在存储桶中都有自己的稳定文件夹：在同一目录上重新运行[sync_job_volume()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.sync_job_volume)只会上传新的或修改的文件。默认情况下，该卷以只读方式安装。
...
将 `ssh=True` 传递给 [run_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.run_job)（或 [run_uv_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.run_uv_job)）以使作业的容器可通过 SSH 访问。 SSH 端点在作业状态中可用：
...
## 配置作业超时作业有默认超时（30 分钟），之后它们将自动停止。在运行模型训练等长时间运行的任务时，了解这一点很重要。
...
使用 [update_job_labels()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.update_job_labels) 替换现有作业上的标签。这将替换所有现有的用户提供的标签：
...
这也适用于[update_scheduled_job_labels()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.update_scheduled_job_labels)的预定作业：
...
将 [create_scheduled_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_scheduled_job) 或 [create_scheduled_uv_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_scheduled_uv_job) 与时间表 `@annually`、`@yearly`、`@monthly`、`@weekly`、`@daily`、`@hourly` 或 CRON 时间表表达式（例如， `"0 9 * * 1"`每周一上午 9 点）：
...
使用与[run_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.run_job)和[run_uv_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.run_uv_job)相同的参数来传递环境变量、秘密、超时等。使用 `list_scheduled_jobs`、[inspect_scheduled_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.inspect_scheduled_job)、[suspend_scheduled_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.suspend_scheduled_job)、[resume_scheduled_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.resume_scheduled_job)、[trigger_scheduled_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.trigger_scheduled_job) 和 [delete_scheduled_job()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.delete_scheduled_job) 管理计划作业：
...
使用 [create_webhook()](/docs/huggingface_hub/v1.27.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_webhook) 创建一个 Webhook，当 Hugging Face 存储库中发生更改时触发作业：
...
### 创建 CLI 扩展
https://huggingface.co/docs/huggingface_hub/v1.27.0/guides/cli-extensions.md