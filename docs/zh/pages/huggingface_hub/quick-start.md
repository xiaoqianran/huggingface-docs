...
或整个存储库。您可以使用[hf_hub_download()](/docs/huggingface_hub/v1.32.0/en/package_reference/file_download#huggingface_hub.hf_hub_download)功能下载文件。
...
有关更多详细信息和选项，请参阅 [hf_hub_download()](/docs/huggingface_hub/v1.32.0/en/package_reference/file_download#huggingface_hub.hf_hub_download) 的 API 参考。
...
最简单的身份验证方法是使用 [login()](/docs/huggingface_hub/v1.32.0/en/package_reference/authentication#huggingface_hub.login) 命令：
...
或者，您可以在笔记本或脚本中使用 [login()](/docs/huggingface_hub/v1.32.0/en/package_reference/authentication#huggingface_hub.login) 以编程方式登录：
...
您一次只能登录一个帐户。登录新帐户后，您将自动退出前一个帐户。要确定您当前的活动帐户，只需运行 `hf auth whoami` 命令。> [!警告]
...
您只需使用每个令牌使用 [login()](/docs/huggingface_hub/v1.32.0/en/package_reference/authentication#huggingface_hub.login) 命令登录即可在计算机上保存多个令牌。如果需要在本地切换这些令牌，可以使用 `auth switch` 命令：
...
注册并登录后，使用 [create_repo()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_repo) 创建一个存储库
功能：
...
## 上传文件使用 [upload_file()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_api#huggingface_hub.HfApi.upload_file) 函数将文件添加到新创建的存储库中。你
需要指定：
