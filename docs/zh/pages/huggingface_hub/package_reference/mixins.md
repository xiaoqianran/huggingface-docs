...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/hub_mixin.py#L66)
...
必须在`_from_pretrained`和`_save_pretrained`中被覆盖。 [PyTorchModelHubMixin](/docs/huggingface_hub/v1.32.0/en/package_reference/mixins#huggingface_hub.PyTorchModelHubMixin)就是一个很好的例子
mixin 与 Hub 的集成。查看我们的[integration guide](../guides/integrations)以获取更多说明。当继承[ModelHubMixin](/docs/huggingface_hub/v1.32.0/en/package_reference/mixins#huggingface_hub.ModelHubMixin)时，可以定义类级别的属性。这些属性不会传递给
`__init__` 但对于类定义本身。这对于定义有关库集成的元数据很有用
[ModelHubMixin](/docs/huggingface_hub/v1.32.0/en/package_reference/mixins#huggingface_hub.ModelHubMixin)。
...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/hub_mixin.py#L440)
...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/hub_mixin.py#L567)
...
model_kwargs ：传递给 [_from_pretrained()](/docs/huggingface_hub/v1.32.0/en/package_reference/mixins#huggingface_hub.ModelHubMixin._from_pretrained) 方法的附加关键字参数。

在子类中覆盖此方法以定义如何从预训练中加载模型。

在加载文件之前，使用 [hf_hub_download()](/docs/huggingface_hub/v1.32.0/en/package_reference/file_download#huggingface_hub.hf_hub_download) 或 [snapshot_download()](/docs/huggingface_hub/v1.32.0/en/package_reference/file_download#huggingface_hub.snapshot_download) 从集线器下载文件。大多数
...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/hub_mixin.py#L451)

**参数：**pretrained_model_name_or_path (`str`, `Path`) : - Hub 上托管的模型的 `model_id`（字符串），例如`bigscience/bloom`。 - 或者包含使用 [save_pretrained](https://huggingface.co/docs/transformers/v5.17.0/en/main_classes/model#transformers.PreTrainedModel.save_pretrained) 保存的模型权重的 `directory` 路径，例如 `../path/to/my_model_directory/`。
...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/hub_mixin.py#L609)**参数：**
...
`delete_patterns` 在同一提交中删除现有的远程文件。更多信息请参见[upload_folder()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_api#huggingface_hub.HfApi.upload_folder)参考
详细信息。
...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/hub_mixin.py#L372)
...
model_card_kwargs (`dict[str, Any]`, *可选*) ：传递给模型卡模板以自定义模型卡的附加参数。Push_to_hub_kwargs ：传递给 [push_to_hub()](/docs/huggingface_hub/v1.32.0/en/package_reference/mixins#huggingface_hub.ModelHubMixin.push_to_hub) 方法的附加关键字参数。
...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/hub_mixin.py#L692)

实现[ModelHubMixin](/docs/huggingface_hub/v1.32.0/en/package_reference/mixins#huggingface_hub.ModelHubMixin)，为 PyTorch 模型提供模型 Hub 上传/下载功能。型号
默认情况下使用`model.eval()`设置为评估模式（辍学模块已停用）。为了训练模型，
您应该首先使用 `model.train()` 将其设置回训练模式。

有关如何使用 mixin 的更多详细信息，请参阅[ModelHubMixin](/docs/huggingface_hub/v1.32.0/en/package_reference/mixins#huggingface_hub.ModelHubMixin)。
...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/fastai_utils.py#L289)
...
[Source](https://github.com/huggingface/huggingface_hub/blob/v1.32.0/src/huggingface_hub/fastai_utils.py#L334)

**参数：**

learner (*Learner*) ：您想要推送到中心的 *fastai.Learner'。repo_id (*str*) ：Hub 中模型的存储库 ID，格式为“namespace/repo_name”。命名空间可以是您的个人帐户或您具有写入权限的组织（例如“stanfordnlp/stanza-de”）。
...
token (*str*, *可选*) ：Hugging Face 帐户令牌，用作远程文件的 HTTP 承载授权。如果`None`，将通过提示询问令牌。
...
api_endpoint (*str*, *可选*) ：将模型推送到中心时要使用的 API 端点。allow_patterns (*list[str]* 或 *str*, *可选*) ：如果提供，则仅推送至少匹配一种模式的文件。
...
### 实用程序
https://huggingface.co/docs/huggingface_hub/v1.32.0/package_reference/utilities.md