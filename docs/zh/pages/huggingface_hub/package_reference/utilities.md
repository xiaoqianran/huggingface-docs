...
您还可以启用或禁用特定组的进度栏。这使您可以在应用程序或库的不同部分中更精细地管理进度条可见性。当某个组的进度条被禁用时，除非明确覆盖，否则该组下的所有子组也会受到影响。
...
如果您在需要自定义配置（例如自定义代理或认证）的特定环境中运行脚本，这可能会很有用。
使用 `get_async_client` 获得正确配置的 `httpx.AsyncClient`。
...
在 https://github.com/huggingface/huggingface_hub/issues/1008 中进行了讨论。
在登月（内部存储库）中：
