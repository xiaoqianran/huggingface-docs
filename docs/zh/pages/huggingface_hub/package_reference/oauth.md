...
- 如果用户登录，则会返回一个包含用户信息的[OAuthInfo](/docs/huggingface_hub/v1.32.0/en/package_reference/oauth#huggingface_hub.OAuthInfo)对象。如果不是，则返回`None`。
...
以 [OAuthInfo](/docs/huggingface_hub/v1.32.0/en/package_reference/oauth#huggingface_hub.OAuthInfo) 对象的形式返回登录用户的信息。
...
有关如何使用此方法的示例，请参阅[attach_huggingface_oauth()](/docs/huggingface_hub/v1.32.0/en/package_reference/oauth#huggingface_hub.attach_huggingface_oauth)。
...
user_info ([OAuthUserInfo](/docs/huggingface_hub/v1.32.0/en/package_reference/oauth#huggingface_hub.OAuthUserInfo)) ：用户信息。

state（`str`，*可选*）：在向 OAuth 提供商发出的原始请求中传递给 OAuth 提供商的状态。
