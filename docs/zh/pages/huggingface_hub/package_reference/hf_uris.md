...
本页记录了 HF URI 和 HF 安装的规范语法。库中的任何地方都使用相同的解析器，因此在一个上下文中有效的 URI（例如 [HfFileSystem](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_file_system#huggingface_hub.HfFileSystem)）在另一个上下文中进行相同的解析。

[parse_hf_uri()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.parse_hf_uri) 还接受 Hugging Face **网络 URL**（例如 `https://huggingface.co/datasets/my-org/my-dataset/blob/main/train.csv`），因此您可以从网站复制粘贴链接。请参阅下面的[Web URLs](#web-urls)。
...
为了方便起见，[parse_hf_uri()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.parse_hf_uri) 还接受您从浏览器复制粘贴的 Hugging Face Web URL。它们被标准化为规范的 `hf://` 形式，因此您可以将 URL 直接粘贴到 CLI 或库中，然后“它就可以工作”：
...
[HfUri.to_url()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.HfUri.to_url) 是解析 URL 的逆过程：它呈现 HF URI 的可浏览 Web URL。
...
```当未设置路径或修订时，它指向存储库/存储桶登录页面；当仅设置修订时，它指向文件夹查看器（`/tree/<rev>`）；对于存储库文件，它指向文件查看器（`/blob/<rev>/<path>`，修订版本默认为`main`）；对于存储桶文件，它指向树路由（`/tree/<path>`）。路径中的特殊字符（空格、`#`、...）采用百分比编码。传递 `endpoint=...` 来定位自定义主机。

## Python 解析

### 解析 URI

[parse_hf_uri()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.parse_hf_uri) 是集中式 URI 解析器。它是一个纯字符串解析器（无网络调用）并返回冻结的 [HfUri](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.HfUri) 数据类。
...
[HfUri](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.HfUri) 可通过 [HfUri.to_uri()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.HfUri.to_uri) 进行往返，它始终发出规范形式（带有显式类型前缀）：
...
`parse_hf_mount` 解析挂载规范（具有本地挂载路径和可选 `:ro`/`:rw` 标志的 HF URI）并返回冻结的 `HfMount` 数据类。它在底层使用[parse_hf_uri()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.parse_hf_uri)。
...
将 URI 呈现为 Hugging Face **web URL**（您在浏览器中打开的那种）。这是使用 [parse_hf_uri()](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.parse_hf_uri) 解析 URL 的逆过程。返回的 URL 指向：
...
**返回：** [HfUri](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.HfUri)
...
source ([HfUri](/docs/huggingface_hub/v1.32.0/en/package_reference/hf_uris#huggingface_hub.HfUri)) ：解析后的 HF URI，标识要挂载的集线器资源。
